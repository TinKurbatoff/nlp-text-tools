...
def main():
    parser = argparse.ArgumentParser(
        description="Compare texts to find common words and their frequencies"
    )

    parser.add_argument(
        "files", nargs="+", type=argparse.FileType("rU"), help="text files to compare, the last one should be the source"
    )
    ...
    parser.add_argument(
        ...
        help="""how to sort output; by word alphabetically,
                        number of occurrences, or by word length (default: {})""".format(
            DEFAULT_SORT
        ),
    )
    ...
    #differences = {word for word in set.difference(*tokens)}
    differences = {word for word in tokens[len(tokens)-1].difference(tokens[:len(tokens)-2])}
    ...
    print("Found {} new words".format(len(differences)))
    ...
    matches = [
        (word, [f.get(word) for f in fdists][-1], len(word)) for word in differences
    ]
